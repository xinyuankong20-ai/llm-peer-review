...
OUTPUT_FILE = r"C:\Users\pc\Desktop\paper\full_126_experiment_results_v2.csv"
...
def run_full_experiment():
    ...
    print("=" * 80)
    print(
        f"Full 6-Dimension Review Started! (Includes Original + 5 Variants, 21 papers x 6 dims x 3 runs)"
    )
    ...
    if os.path.exists(OUTPUT_FILE):
        try:
            ...
            print(f"Loaded existing records: {len(all_records)} items.")
        except Exception:
            pass
    ...
    for p_idx, paper in enumerate(paper_prefixes, 1):
        ...
        for variant in VARIANTS:
            ...
            for rep in range(1, 4):
                ...
                print(
                    f"[{actual_file[:35]}...] -> [{variant}] Run {rep}/3 calling..."
                )
                ...
                if result:
                    ...
                    print(
                        f"Total Score: [{result.total_score}] | Sub-scores: [Nov:{result.novelty} | Rig:{result.methodological_rigour} | Cre:{result.result_credibility} | Cla:{result.writing_clarity}]"
                    )
                else:
    ...
    print("\n" + "=" * 80)
    print(f"Full 6-Dimension Experiment Successfully Completed! Results saved to: {OUTPUT_FILE}")
    print("=" * 80)
